// NBU official exchange rates — pure data layer. Public JSON API, no key.
const PROXIES = [
...
async function via(url) {
  ...
  for (const w of PROXIES) {
    ...
    try {
      const r = await fetch(w(url), { signal: c.signal });
      if (!r.ok) throw 0;
  ...
  throw err || new Error("nbu unavailable");
}

export async function fetchRates() {
  const data = JSON.parse(await via(API));
  const rates = data.map((d) => ({ cc: d.cc, name: d.txt, rate: d.rate, date: d.exchangedate }));
  rates.sort((a, b) => {
    const pa = POPULAR.indexOf(a.cc), pb = POPULAR.indexOf(b.cc);
    return (pa < 0 ? 99 : pa) - (pb < 0 ? 99 : pb) || a.cc.localeCompare(b.cc);
  });
  return { rates, date: rates[0]?.date || "" };
}
